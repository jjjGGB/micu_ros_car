#include "lidar.h"



typedef struct { //lidar激光雷达解码数据结构体
  uint16_t rotate_speed; //雷达转速
  uint16_t Initial_Angle; //雷达起始角度
  uint16_t end_Angle; //雷达结束角度
  uint16_t timestamp; //时间戳
} lidar_radar_t;

lidar_radar_t lidar_radar_data; //lidar数据结构体


uint8_t lidar_data_buf[200] = { 0 };  //lidar数据缓冲区

static const uint8_t CrcTable[256] = {//CRC校验表
  
};

uint8_t CalCRC8(const uint8_t* data, uint16_t data_len) { //CRC校验
  uint8_t crc = 0;
  while (data_len--) {
    crc = CrcTable[(crc ^ *data) & 0xff];
    data++;
  }
  return crc;
}


int lidar_test(void){ //YDLIDAR X2激光雷达检测
	uint8_t re_Buf[80] = {0};
	const int rxBytes = uart_read_bytes(UART_NUM_1, re_Buf, 60, 0);

	printf(" 接收到 %d 字节  \n",rxBytes);
	if (rxBytes >= 47) {//如果读取到数据就进行校验
		for(int i = 0 ; i < 60 ; i++){
			printf("%x ",re_Buf[i]);
		}
		printf("\n");
		if(re_Buf[0] == 0xAA && re_Buf[1] == 0x55) {//CRC校验
			printf("CRC校验 成功\n");
			return ESP_OK;
		}else{
			printf("CRC校验 失败\n");
			uart_flush(UART_NUM_1);
			return ESP_FAIL;
		}
	}
	return ESP_FAIL;
}



void lidar_data_Task(void *pvParameter)
{
	while (1) 
	{
	  const int rxBytes = uart_read_bytes(UART_NUM_1, lidar_data_buf, 47, 0);  // 直接使用0作为超时时间

		if (rxBytes > 0) 
		{
			udp_write_lidar(lidar_data_buf,rxBytes);
		}
				
		vTaskDelay(1 / portTICK_PERIOD_MS); //读取后释放1个周期给看门狗复位
	}
		
}



void  lidar_task(void)
{
    //xTaskCreate(lidar_data_Task, "lidar_data_Task", 4096 * 8, NULL, 15, NULL);
}



