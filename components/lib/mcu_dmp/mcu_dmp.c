...
static float Kp = 12.00f;                               /* 比例增益（用于静止状态） */
...
static const float STATIC_THRESHOLD = 0.6f;  // 静止检测阈值
...
void imu_update(Axis3f acc, Axis3f gyro, float dt)
{
    ...
    //printf("静止检测gyro_magnitude:%.2f",gyro_magnitude);
    ...
    if ((acc.x != 0.0f) || (acc.y != 0.0f) || (acc.z != 0.0f))
    {
        ...
        if (gyro_magnitude < STATIC_THRESHOLD) {
            ...
            gyro.y += Kp * ey + eyInt;
            gyro.z += Kp * ez + ezInt;
            //printf("PI修正后gyro_:%.2f,%.2f,%.2f\n",gyro.x,gyro.y,gyro.z);
        } else {
            
             //printf("处于运动，使用Madgwick算法");
            /* 运动状态使用Madgwick算法 */
            /* 计算四元数变化率 */
